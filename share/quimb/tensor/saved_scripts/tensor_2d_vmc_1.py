...
def set_options(deterministic=False,backend='numpy',**compress_opts):
...
class ContractionEngine:
    def init_contraction(self,Lx,Ly,phys_dim=2):
        ...
        for i in range(phys_dim):
            ...
            data[i] = 1.
            self.data_map[i] = self._2backend(data,False)
    ...
    def get_bra_tsr(self,peps,ci,i,j,append=''):
        ...
        return Tensor(data=self.data_map[ci],inds=inds,tags=tags)
...
class AmplitudeFactory(ContractionEngine):
    ...
    def dict2vec(self,dict_):
        return jnp.concatenate(self.dict2vecs(dict_))
    ...
    def psi2vec(self,psi=None):
        return jnp.concatenate(self.psi2vecs(psi)) 
    def get_x(self):
        return self._2numpy(self.psi2vec())
    def split_vec(self,x):
        ...
        for ix,(_,size,_) in enumerate(self.constructors):
            ...
            start = stop
        return ls 
    def vec2tensor(self,x,ix,requires_grad):
        shape = self.constructors[ix][0]
        return self._2backend(x.reshape(shape),requires_grad)
    def vec2dict(self,x): 
        dict_ = dict() 
        ls = self.split_vec(x)
    ...
    def vec2psi(self,x,inplace=True): 
        ...
        ls = self.split_vec(x)
        for ix,(_,_,site) in enumerate(self.constructors):
            psi[psi.site_tag(*site)].modify(data=self.vec2tensor(ls[ix],ix,True))
        return psi
        ...
        for ((i0,j0),(x_bsz,y_bsz)),tn in plq.items():
            for i in range(i0,i0+x_bsz):
                for j in range(j0,j0+y_bsz):
                    ...
                    vx[i,j] = self.site_grad(tn.copy(),i,j)/cij
    ...
    def prob(self, config):
        ...
        """
        return self._2numpy(self.unsigned_amplitude(config)) ** 2
####################################################################################
# ham class 
####################################################################################
class Hamiltonian(ContractionEngine):
    ...
    def pair_tensor(self,bixs,kixs,tags=None):
        ...
        return Tensor(data=self.data_map[self.key],inds=inds,tags=tags) 
...
class Heisenberg(Hamiltonian):
    def __init__(self,J,h,Lx,Ly,**kwargs):
        ...
        self.key = 'Jxy'
        self.data_map[self.key] = self._2backend(data,False)
