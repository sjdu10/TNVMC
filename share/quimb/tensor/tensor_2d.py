...
class TensorNetwork2D(TensorNetwork):
    ...
    def compress_row(
        ...
        if len(self[0,0].shape) < 4:
            ...
            if sweep == 'right':
                for j in range(min(yrange), max(yrange)+1, +1):
                    self.compress_between((i, j), (i, (j + 1)%self.Ly), max_bond=max_bond,
                                        cutoff=cutoff, **compress_opts)
            else:
                for j in range(max(yrange)+1, min(yrange), -1):
                    self.compress_between((i, j), (i, (j - 1)%self.Ly), max_bond=max_bond,
                                        cutoff=cutoff, **compress_opts)
    ...
    def compress_column(
        ...
        if len(self[0,0].shape) < 4: # OBC
            ...
            if sweep == 'up':
                for i in range(min(xrange), max(xrange)+1, +1):
                    self.compress_between((i, j), ((i + 1)%self.Lx, j), max_bond=max_bond,
                                        cutoff=cutoff, **compress_opts)
            else:
                for i in range(max(xrange)+1, min(xrange), -1):
                    self.compress_between((i, j), ((i - 1)%self.Lx, j), max_bond=max_bond,
                                        cutoff=cutoff, **compress_opts)
