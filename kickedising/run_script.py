        ...
        for n in n_list:
            for h in h_list:
                    for depth in depth_list:
                        for J in J_list:
                            for g in g_list:
                                # os.system(f"mpirun -np 14 python run_tnfunc_y.py {n} {depth} {chi} {J} {g} {h}")
                                # os.system(f'sbatch -J n{n}c{chi}d{depth} run.job {n} {depth} {chi} {J} {g} {h} {cd}')
                                os.system(f'sbatch -J n{n}c{chi}d{depth} run_y.job {n} {depth} {chi} {J} {g} {h} {cd}')