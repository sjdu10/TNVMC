...
def amplitude(peps_psi, chi, inds_values, inds=None, direction='xmax'):
    ...
    if direction == 'xmax':
        ...
        if len(peps_config.tensors) != len(inds_values):
            raise ValueError('The input TNS is not an MPS.')
        # amplitude = peps_config.contract_boundary_from_xmin(xrange=(0, 1), max_bond=chi).contract()
        amplitude_ = peps_config.contract() # Assuming peps_config is already in MPS form
    elif direction == 'ymax':
...
def MPS_contract_psi(psi, max_bond):
    psi = psi.copy()
    psi = psi.contract_boundary_from_xmin(xrange=(0, psi.Lx-1), max_bond=max_bond, canonize=True).contract()
    return psi
...
def amplitudes(n, depth, chi, direction='xmax', J=1.0, g=1.0, h=0.0):
    """Non-parallel calculation of amplitudes"""
    import time
