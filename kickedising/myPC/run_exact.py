...
exact_psi = exactly_contract_psi(peps_psi, depth)
...
if RANK == 0:
    ...
    if ee_spectrum:
        entanglement_spectrum_A = entanglement_spectrum(rho_A, 20)
    ...
    # Save the results (depth, S_A) to a txt file
    with open(f'./results/TNexact_n={n}_J={J}_g={g}_h={h}.txt', 'a') as f:
        f.write(f'depth={depth}, S_A={S_A}, chi=-1\n')
    ...
    while not terminate:
        ...
        if config is not None:
            amp = amplitude_exact(exact_psi, config)
